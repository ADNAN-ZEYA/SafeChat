...
export default function Sidebar({
  ...
  const handleNav = (name, onClick) => { setActive(name); onClick?.(); };
  ...
  return (
    <>
      {/* DESKTOP SIDEBAR */}
      <div className="hidden md:flex relative flex-col h-full w-full gap-8 bg-sc-container-low border-r border-sc-outline/25">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button key={item.name} onClick={() => handleNav(item.name, item.onClick)}
                className={[
                  'group flex items-center gap-4 rounded-2xl px-4 py-3 text-[15px] font-medium transition-all duration-200 relative overflow-hidden',
                  isActive ? 'bg-sc-secondary/40 text-sc-primary border border-sc-primary/15 shadow-sm' : 'text-sc-text-muted hover:bg-sc-container-high/60 hover:text-sc-text border border-transparent',
      ...
      {/* MOBILE BOTTOM NAV */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-[100] flex items-center justify-around bg-sc-container-low border-t border-sc-outline/25 px-1 py-2">
        {navItems.map((item) => {
          const isActive = active === item.name;
          return (
            <button key={item.name}
              ...
              <item.icon className={['h-6 w-6 shrink-0', isActive ? 'text-sc-primary' : 'text-sc-text-muted'].join(' ')} />
              <span className={['text-[9px] font-medium truncate', isActive ? 'text-sc-primary' : 'text-sc-text-muted'].join(' ')}>
                {item.name === 'Find Friends' ? 'Friends' : item.name === 'Notifications' ? 'Alerts' : item.name}
              </span>
            </button>
          );
        })}
