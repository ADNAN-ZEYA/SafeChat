    ...
    <div className="relative min-h-screen bg-sc-surface text-sc-text">
      <div className="mx-auto flex max-w-7xl w-full">
        ...
        <aside className="hidden md:block sticky top-0 h-screen w-1/4 shrink-0">
          ...
          <div className="absolute bottom-4 left-0 w-full px-3 space-y-1">
            <button onClick={() => onNavigateToHome()} className="group flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-sc-text-muted transition-all hover:bg-sc-primary-light/15 hover:text-sc-primary">
        ...
        {/* MAIN */}
        <main className="flex min-h-screen flex-1 min-w-0 flex-col pb-24 md:pb-0 bg-sc-container-low border-l border-sc-outline/20">
          <div className="px-4 md:px-8 pt-8 pb-2 animate-fade-in-up">
            <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight text-sc-text">Your Profile</h2>
          ...
          {loading ? (
            ...
            <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
              <div className="mx-auto max-w-xl space-y-6">
                ...
                {/* Avatar */}
                <section className="rounded-card bg-sc-container p-6 md:p-8 elevation-2 animate-fade-in-up">
                  <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-start">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <form onSubmit={handleChangePicture} className="mt-4 flex flex-col gap-3">
                        <input type="text" value={avatarInput} onChange={(e) => setAvatarInput(e.target.value)} placeholder="Paste image URL…"
                          className="w-full rounded-2xl bg-sc-container-high px-4 py-3 text-sm text-sc-text placeholder-sc-text-muted/50 outline-none border border-sc-outline/30 transition-all focus:bg-sc-container-floor focus:border-sc-primary/40 focus:shadow-glow-coral" />
                ...
                {/* Bio */}
                <section className="rounded-card bg-sc-container p-6 md:p-8 elevation-2 animate-fade-in-up stagger-1">
                  ...
                  {editingBio ? (
                    <form onSubmit={handleUpdateBio} className="space-y-4">
                      <textarea value={bioInput} onChange={(e) => setBioInput(e.target.value)} rows={3} maxLength={200} placeholder="Tell others about yourself…"
                        className="w-full rounded-2xl bg-sc-container-high px-4 py-3 text-sm text-sc-text placeholder-sc-text-muted/50 outline-none resize-none border border-sc-outline/30 transition-all focus:bg-sc-container-floor focus:border-sc-primary/40 focus:shadow-glow-coral" />
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <button type="button" onClick={() => { setEditingBio(false); setBioInput(profile?.bio || ''); }}
                            className="rounded-full px-4 py-2 text-sm font-medium text-sc-text-muted border border-sc-outline/25 transition hover:bg-sc-container-high">Cancel</button>
                          <button type="submit" disabled={savingBio || bioInput === profile?.bio}
                            className="rounded-full bg-gradient-primary px-4 py-2 text-sm font-semibold text-sc-on-primary hover-scale border border-sc-primary/20 transition-all disabled:opacity-40">
                    ...
                    <div className="rounded-2xl bg-sc-container-low px-4 py-3 border border-sc-outline/15">
                ...
                {/* Stats */}
                <section className="grid grid-cols-3 gap-3 animate-fade-in-up stagger-2">
                  {[{ label: 'Messages', value: '—' }, { label: 'Friends', value: '—' }, { label: 'Status', value: 'Online' }].map((stat) => (
                    <div key={stat.label} className="rounded-card bg-sc-container px-4 py-5 text-center elevation-1 hover-lift">
