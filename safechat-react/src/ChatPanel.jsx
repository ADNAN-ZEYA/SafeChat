...
export default function ChatPanel({ onClose, currentUser, showNotification, initialActiveUser = null, refreshToken = 0 }) {
  ...
  const filteredUsers = useMemo(() => { const kw = userSearch.trim().toLowerCase(); if (!kw) return availableUsers; return availableUsers.filter((u) => u.toLowerCase().includes(kw)); }, [availableUsers, userSearch]);
  const scrollToBottom = () => { chatWindowRef.current?.scrollTo({ top: chatWindowRef.current.scrollHeight, behavior: 'auto' }); };
  const fetchMessages = useCallback(async () => {
    if (!currentUser || !activeUser || isFetchingMessagesRef.current) return; isFetchingMessagesRef.current = true;
    try { const fetched = await api.getMessages(currentUser, activeUser); if (Array.isArray(fetched)) setMessages(fetched); } catch (e) { console.error('Failed to fetch messages:', e); } finally { isFetchingMessagesRef.current = false; }
  }, [currentUser, activeUser]);
  const scheduleFetchMessages = useCallback(() => { if (messageRefreshTimeoutRef.current) return; messageRefreshTimeoutRef.current = setTimeout(() => { messageRefreshTimeoutRef.current = null; fetchMessages(); }, 250); }, [fetchMessages]);
  const sendTypingEvent = useCallback(async (isTyping) => { const ch = typingChannelRef.current; if (!ch || !currentUser || !activeUser) return; try { await ch.send({ type: 'broadcast', event: 'typing', payload: { from: currentUser, to: activeUser, isTyping, at: Date.now() } }); isTypingBroadcastedRef.current = isTyping; } catch {} }, [currentUser, activeUser]);
  const fetchUsers = useCallback(async () => {
    if (!currentUser) return;
    try { const users = await api.getUsers(currentUser); if (!Array.isArray(users)) return; const userList = users.map((u) => u.username).filter(Boolean); setAvailableUsers(userList); setActiveUser((prev) => { if (initialActiveUser && userList.includes(initialActiveUser)) return initialActiveUser; if (prev && userList.includes(prev)) return prev; return userList[0] || ''; }); } catch (e) { console.error('Failed:', e); }
  }, [currentUser, initialActiveUser]);

  useEffect(() => { fetchUsers(); }, [fetchUsers]);
  useEffect(() => { fetchMessages(); if (!supabase || !supabaseRealtimeEnabled || !currentUser) return; const ch = supabase.channel(`chat_messages_feed_${currentUser}`).on('postgres_changes', { event: '*', schema: 'public', table: 'chat_messages' }, () => scheduleFetchMessages()).on('broadcast', { event: 'typing' }, ({ payload }) => { if (payload?.from === activeUser && payload?.to === currentUser) setIsOtherUserTyping(Boolean(payload.isTyping)); }).subscribe(); typingChannelRef.current = ch; return () => { typingChannelRef.current = null; supabase.removeChannel(ch); }; }, [currentUser, activeUser, fetchMessages, scheduleFetchMessages]);
  useEffect(() => { if (!activeUser) { setIsOtherUserTyping(false); return; } if (!newMessage.trim()) { if (isTypingBroadcastedRef.current) sendTypingEvent(false); return; } if (!isTypingBroadcastedRef.current) sendTypingEvent(true); if (typingBroadcastTimeoutRef.current) clearTimeout(typingBroadcastTimeoutRef.current); typingBroadcastTimeoutRef.current = setTimeout(() => sendTypingEvent(false), 1200); return () => { if (typingBroadcastTimeoutRef.current) clearTimeout(typingBroadcastTimeoutRef.current); }; }, [newMessage, activeUser, sendTypingEvent]);
  useEffect(() => { return () => { if (typingBroadcastTimeoutRef.current) clearTimeout(typingBroadcastTimeoutRef.current); if (messageRefreshTimeoutRef.current) clearTimeout(messageRefreshTimeoutRef.current); if (isTypingBroadcastedRef.current) sendTypingEvent(false); }; }, [sendTypingEvent]);
  ...
  const handleSendMessage = async (e) => { e.preventDefault(); if (!newMessage.trim() || !activeUser) return; try { const r = await api.sendMessage(currentUser, activeUser, newMessage); if (r.notification) showNotification(r.notification); if (Array.isArray(r.messages)) setMessages(r.messages); setNewMessage(''); if (isTypingBroadcastedRef.current) sendTypingEvent(false); scheduleFetchMessages(); } catch (e) { showNotification(`Error: ${e.message}`); } };
  ...
  const submitReport = async (e) => { e.preventDefault(); if (!reportTarget) return; setIsSubmittingReport(true); try { await api.reportMessage(currentUser, reportTarget.id, reportReason, reportDescription.trim() || null); showNotification('Message reported.'); closeReportDialog(); } catch (err) { showNotification(`Error: ${err.message}`); } finally { setIsSubmittingReport(false); } };
        ...
        <div className="flex h-[85vh] w-[92vw] max-w-6xl overflow-hidden rounded-card bg-sc-container-floor elevation-4 animate-scale-in" onClick={(e) => e.stopPropagation()}>
                ...
                return (
                  <button key={username} onClick={() => setActiveUser(username)}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-sc-text-muted">{isActive ? 'Active now' : 'Tap to chat'}</p>
                    </div>
          ...
          <section className="flex min-w-0 flex-1 flex-col bg-sc-container-floor">
            <header className="flex items-center justify-between px-6 py-4 border-b border-sc-outline/20 bg-sc-container-low/40">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-sc-container-floor bg-sc-primary" />
                </div>
                <div>
                  <p className="text-sm font-semibold capitalize text-sc-text">{activeUser}</p>
                  <p className="text-[11px] text-sc-text-muted">{isOtherUserTyping ? <span className="flex items-center gap-1 text-sc-primary">typing <TypingDots /></span> : 'Online'}</p>
                </div>
              </div>
            ...
            <div ref={chatWindowRef} className="flex-1 space-y-3 overflow-y-auto px-6 py-5">
              {Array.isArray(messages) && messages.length === 0 && <div className="flex h-full items-center justify-center text-sm text-sc-text-muted">No messages yet. Say hello! 👋</div>}
              {Array.isArray(messages) && messages.map((msg, idx) => {
                const isMine = msg.user === currentUser;
                return (
                  <div key={msg.id ?? idx} className={`group flex ${isMine ? 'justify-end' : 'justify-start'} animate-fade-in-up`} style={{ animationDelay: `${idx * 30}ms` }}>
                    <div className="relative max-w-[65%]">
                      ...
                      <div className={`rounded-2xl px-4 py-2.5 text-sm border transition ${isMine ? 'bg-gradient-primary text-sc-on-primary rounded-br-md border-sc-primary/20 shadow-glow-coral' : 'bg-sc-container text-sc-text rounded-bl-md border-sc-outline/20'}`}>
                        <p className="leading-relaxed">{msg.text}</p>
                        <p className={`mt-1 text-right text-[10px] ${isMine ? 'text-sc-on-primary/60' : 'text-sc-text-muted'}`}>{formatTime(msg.created_at)}</p>
                      </div>
                    </div>
                  </div>
                );
              })}
              {isOtherUserTyping && <div className="flex justify-start"><span className="rounded-2xl rounded-bl-md bg-sc-container px-4 py-2 text-xs text-sc-text-muted border border-sc-outline/20">typing<TypingDots /></span></div>}
            </div>
