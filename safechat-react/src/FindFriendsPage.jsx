    ...
    <div className="relative min-h-screen text-sc-text bg-sc-surface">
      <div className="mx-auto flex max-w-7xl w-full">
        ...
        <main className="flex min-h-screen flex-1 min-w-0 flex-col pb-24 md:pb-0 bg-sc-container-low border-l border-sc-outline/20">
          <div className="px-4 md:px-8 pt-8 pb-4 animate-fade-in-up">
            <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight text-sc-text">Find Friends</h2>
            <p className="mt-2 text-sm text-sc-text-muted">Browse registered users and start chatting instantly.</p>
          </div>
          <div className="px-4 md:px-8 pb-4 animate-fade-in-up stagger-1">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {filteredUsers.map((item, idx) => (
                  <div key={item.username}
                    className="group flex items-center gap-4 rounded-card bg-sc-container p-4 md:p-5 elevation-1 hover-lift animate-fade-in-up"
                    ...
                    <button onClick={() => onStartChat(item.username)}
                      className="shrink-0 rounded-full bg-sc-secondary px-4 py-2 text-xs font-semibold text-sc-on-secondary hover-scale border border-sc-on-secondary/15 transition-all">
